...
def base_agg_model_evaluation(model, data_loader, num_cluster):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for x, target in data_loader:
            ...
            label_true[ii * data_loader.batch_size:(ii + 1)
                       * data_loader.batch_size] = target.numpy()
            ii = ii + 1
        assignment = agg.fit_predict(datas)
        ACC = acc(label_true, assignment, num_cluster)
        NMI = nmi(label_true, assignment)
        ARI = ari(label_true, assignment)
        print('image_ACC', ACC)
        print('image_NMI', NMI)
        print('image_ARI', ARI)
        return assignment, ACC, NMI, ARI
