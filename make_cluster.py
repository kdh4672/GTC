...
import json
import clip
from Metrics import base_kmeans_model_evaluation, kmeans_with_init,base_agg_model_evaluation
...
if dataset_name == 'mit':
    is_test_dataset = True
    testset = custom_dataset(root_dir='/database/daehyeon/mit', transform=preprocess)
    testloader = DataLoader(
        dataset=testset, 
        batch_size=batch_size,
        shuffle=False)
...
if is_test_dataset:
    ...
    cluster_name = 'test_cluster.npy'
else:
    dataloader = concatloader
    cluster_name = 'concat_cluster.npy'
print('------------------')
print(backbone_name)
...
with torch.no_grad():
    if args.cluster == 'kmean':
        centroids, cluster_label, acc, nmi, ari= base_kmeans_model_evaluation(
        model, dataloader, num_classes)
        npy_file_path = '_'.join(['npy_folder/',dataset_name,backbone_name.replace('/','_'),cluster_name])        
    elif args.cluster == 'agg':
        cluster_label, acc, nmi, ari= base_agg_model_evaluation(
        model, dataloader, num_classes)
        npy_file_path = '_'.join(['npy_folder/',dataset_name,backbone_name.replace('/','_'),args.cluster,cluster_name])

np.save(npy_file_path , cluster_label)

file_path = 'npy_folder/model_acc_nmi.json'

with open(file_path,'r') as f:
    acc_nmi_dict = json.load(f)
    acc_nmi_dict[npy_file_path] = [acc,nmi,ari]

with open(file_path,'w') as f:
    json.dump(acc_nmi_dict,f,indent=4)