...
# new cluster assignments
    if is_test_dataset:
        print('new_cluter_change')
        dataset.targets = new_label.tolist()
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=batch_size, shuffle=True)
    else:
        train_c_set.targets = new_label[:len(train_c_set)].tolist()
        test_c_set.targets = new_label[len(train_c_set):].tolist()
        dataset = torch.utils.data.ConcatDataset([train_c_set, test_c_set])
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=batch_size, shuffle=True)
